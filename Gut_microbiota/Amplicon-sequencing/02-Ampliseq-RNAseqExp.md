RNA-sequencing experiment: gut microbiota analyses
================
...
``` bash
cd /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/RawFiles/
...
``` bash
mkdir /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis
mkdir /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/00_FASTQC/
  cd /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/RawFiles
for f in *.fastq.gz; do 
fastqc $f -o /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/00_FASTQC/; 
done
```
...
``` bash
mkdir /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/02_dada2
cd /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/RawFiles
...
``` bash
mkdir /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/01_trimmed
cd /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/RawFiles
for f in *cut.fastq.gz; do mv $f /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/01_trimmed/$f; done
cd /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/01_trimmed
for f in *cut.fastq.gz; do mv $f ${f/L1_/L1.}; done
```
...
raw=`echo $(zless /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/RawFiles/$f.R1.fastq.gz | wc -l)/4|bc`
cut=`echo $(zless /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/01_trimmed/$f.R1.cut.fastq.gz | wc -l)/4|bc`
...
``` r
path <- "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/02_dada2"
knitr::opts_knit$set(root.dir = normalizePath(path)) 
```

``` r
library(dada2); packageVersion("dada2")
pathraw <-"/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/01_trimmed"
...
``` r
path<-"/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/01_trimmed"
dev.new()
...
plotQualityProfile(fnFs[121:136])
...
plotQualityProfile(fnRs[121:136])
```
...
taxa <- assignTaxonomy(seqtab.nochim, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/silva_nr99_v138_train_set.fa.gz", multithread=TRUE)
taxa <- addSpecies(taxa, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/silva_species_assignment_v138.fa.gz")
...
``` bash
mkdir /Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy
```

``` r
pathtax <- "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/"
...
``` r
uniquesToFasta(seqtab.nochim, fout='/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/seqtab.nochim.fasta', ids=colnames(seqtab.nochim))
```
...
``` r
unqs.mock <- seqtab.nochim["MOCK_1",] # or MOCK_2
unqs.mock <- sort(unqs.mock[unqs.mock>0], decreasing=TRUE) # Drop ASVs absent in the Mock
cat("DADA2 inferred", length(unqs.mock), "sample sequences present in the Mock community.\n")

mock.ref <- getSequences(file.path(path, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/mock_seq_plasmids.fasta"))
...
setwd("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/")

# Set plotting theme
theme_set(theme_bw())

#Data frame containing sample information
samdf = read.table("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/GutMicrobiota_RNAseqExp_metadata.txt", header = T, fill=TRUE) # fill=TRUE allows to read a table with missing entries
...
ps.mit <- subset_taxa(ps.raw, Family=="Mitochondria", prunesamples=TRUE) # Depending on the classifier you used, double check if your Chloroplasts or Mitochondria are in the Order and Family levels...
tax_table(ps.mit)
...
write.table(table, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/RawASVtable_dada2taxa.txt", sep="\t")
```
...
``` r
saveRDS(ps, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/ps_dada2taxa_BrainRNAseq.rds")
...
names(rs) <- new_names
rs # inspect
Biostrings::writeXStringSet(rs, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/AllRawASVs.fasta")
```
...
``` r
bl <- read.table("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/AllRawASVs_Blast.txt", header=F, sep = "\t")
bl <- bl[!duplicated(bl$V1),]
write.table(bl, "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/AllRawASVs_Blast_OnlyFirstHit.txt", sep = "\t")
write.table(tax_table(ps), "/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/TaxTable_ps.txt", sep = "\t")
```
...
ps <- readRDS("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/ps_dada2taxa_BrainRNAseq.rds")
tax_tab <- read.table("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/analysis/03_Taxonomy/TaxTable_ps_RNAseqExp_Annotated.txt", header = T, sep = "\t") # read in the manually annotated taxonomy
row.names(tax_tab) <- tax_tab[,1]
tax_tab[,1] <- NULL
tax_table(ps) <- as.matrix(tax_tab)

#Data frame containing sample information
samdf = read.table("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/GutMicrobiota_RNAseqExp_metadata.txt", header = T, fill=TRUE, na.strings=("")) # fill=TRUE allows to read a table with missing entries
rownames(samdf) = samdf$Sample_ID
```
...
``` r
#load("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/BrainRNAseq_Ampliseq.rdata")
```
...
``` r
mdf = psmelt(Filtered)
mdf <- mdf[order(factor(mdf$Treatment, levels = c("MD", "CL_Bifi", "CL_13", "CL", "inoculum", "blank", "H2O", "mock"), ordered=TRUE), mdf$Sample), ]  # Let's first sort by Treatment, then hive, then sample ID
...
set.seed(7) # Here we use set.seed to make the colors reproducible, because in the scale_fill_manual we use "sample" to shuffle the order of colors of the palette
p1 = ggplot(mdf, aes_string(x = "Sample", y = "Abundance", fill = "Annotated")) +
        geom_bar(stat = "identity", position = "stack", color = "black") +
        ylab("Relative abundance") +
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="bottom") +
        scale_fill_manual(values=sample(cbPalette(55))) 
...
p2 = ggplot(mdf.blanks, aes_string(x = "Sample", y = "Abundance", fill = "Annotated")) +
  ...
  xlab(element_blank()) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="bottom") +
  scale_fill_manual(values=cbPalette(13)) 
...
p5 = ggplot(mdf.inoculum, aes_string(x = "Sample", y = "Abundance", fill = "Annotated")) +
  ...
  xlab(element_blank()) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="bottom") +
  scale_fill_manual(values=cbPalette(28)) 
...
plot_grid(p2, p3, p4, p5, labels = c("A", "B", "C", "D"), ncol = 1, nrow = 4, rel_widths=c(2,2,2,2))
dev.off()
```
...
## Remove ASVs that come from a different project sequenced in the same MiSeq run
...
badTaxa = c("ASV50") # Candidatus Blochmannia, an ant associate from the Camponotus fellah ants run on the same machine. ASV found in traces here due to index swapping
ps.noncontam = pop_taxa(ps, badTaxa)
```

## Remove non experimental samples before replotting qPCR and MiSeq results

``` r
ps.noncontam.filt <- prune_samples(ps.noncontam@sam_data$Treatment == "MD" | ps.noncontam@sam_data$Treatment == "CL_Bifi" | ps.noncontam@sam_data$Treatment == "CL_13" | ps.noncontam@sam_data$Treatment == "CL", ps.noncontam)
ps.noncontam.filt = filter_taxa(ps.noncontam.filt, function(x) sum(x) > 0, TRUE)
ps.noncontam.filt
ps.noncontam.filt.wInocula <- prune_samples(ps.noncontam@sam_data$Treatment == "MD" | ps.noncontam@sam_data$Treatment == "CL_Bifi" | ps.noncontam@sam_data$Treatment == "CL_13" | ps.noncontam@sam_data$Treatment == "CL" | ps.noncontam@sam_data$Treatment == "inoculum", ps.noncontam)
...
## Retain only OTUs that have at least 1% relative abundance in minimum 2 samples
...
p2 = ggplot(mdf, aes_string(x = "Sample", y = "Abundance", fill = "Annotated")) +
        geom_bar(stat = "identity", position = "stack", color = "black") +
        ylab("Relative abundance") +
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="bottom") +
        scale_fill_manual(values=sample(cbPalette(14)))

p3 = ggplot(mdf, aes_string(x = "Sample", y = "Abundance", fill = "Annotated")) +
        geom_bar(stat = "identity", position = "stack", color = "black") +
        ylab("Relative abundance") +
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="bottom", strip.background = element_blank(),
...
plot_grid(p1, p3, ncol = 1, nrow = , rel_widths=c(2,2), rel_heights = c(2,6), align = 'v')
...
ratios <- ratios[,11:length(ratios)]
...
``` r
# Ordinate using Principal Coordinate analysis of Bray-Curtis dissimilarities 
bee_pcoa <- ordinate(
  physeq = ps_copies, 
  method = "PCoA", 
  distance = "bray", 
...
``` r
save.image("/Volumes/My\ Passport\ for\ Mac/MiSeqRun_Analyses/BRAINproject/DADA2/BrainRNAseq_Ampliseq.rdata")
```
